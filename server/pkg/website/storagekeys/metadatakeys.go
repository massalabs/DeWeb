...
func GlobalMetadataKey(metadataKey []byte) []byte {
	return append([]byte{GLOBAL_METADATA_TAG}, metadataKey...)
}
...
func FileMetadataKey(hashLocation []byte, metadataKey ...[]byte) []byte {
	if len(metadataKey) == 0 {
		return append([]byte{FILE_METADATA_TAG}, hashLocation...)
	}

	return append(append([]byte{FILE_METADATA_TAG}, hashLocation...), metadataKey[0]...)
}
...
func FileMetadataLocationKey(hashLocation []byte) []byte {
	return FileMetadataKey(append([]byte{FILE_METADATA_LOCATION_TAG}, hashLocation...))
}
