...
describe('createBatches', () => {
  it('should create batches with the correct number of elements', () => {
    const files = Array.from(
      { length: batchSize * 2 + 1 },
      (_, i) => new FileInit(`file${i}`)
    )
    const filesToDelete = Array.from(
      { length: batchSize * 2 - 1 },
      (_, i) => new FileDelete(`hash${i}`)
    )
    const metadatas = Array.from(
      { length: batchSize },
      (_, i) => new Metadata(`key${i}`, `value${i}`)
    )
    const metadatasToDelete = Array.from(
      { length: 2 },
      (_, i) => new Metadata(`key${i}`, `value${i}`)
    )
  ...
  it('should handle inputs with same lengths', () => {
    ...
    const metadatas = Array.from({ length: 2 }, () => new Metadata())
    const metadatasToDelete = Array.from(
      { length: 2 },
      (_, i) => new Metadata(`key${i}`)
    )
